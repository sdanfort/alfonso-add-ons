%-------------------------------------------------------------------------------
% USAGE of "demo_cputime_pdint".
%
% In this demo, we compute the matrix of coefficients C0f by FFT and MM method
% (see papers on Padua points for notations and details), comparing the cputime.
%-------------------------------------------------------------------------------
...
for index=1:length(n_vett)
    ...
    for index=1:number_test
        C0f = pdcfsFFT(Pad,@funct,xyrange);
    end
    t(4)=cputime;
    ...
    t(1)=cputime;
    for index=1:number_test
        C0f = pdcfsMM(X1,Y1,X2,Y2,@funct,xyrange);
    end
    t(2)=cputime;
